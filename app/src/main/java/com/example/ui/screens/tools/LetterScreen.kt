        ...
        LazyColumn(
            ...
            item {
                Box(
                    ...
                    ResultDisplayCard(
                        resultText = if (hasGenerated) currentResult else "?",
                        detailsText = if (hasGenerated) "$letterCount letter${if (letterCount > 1) "s" else ""} • ${if (uppercase) "Uppercase" else "Lowercase"}" else "Tap Generate Random Letter to start",
